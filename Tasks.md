...
/feature-dev:feature-dev Create a new ZIP/folder based project file type .spa "Swim Animator Project" which will be based on a zipped folder but optionally be read and written as folder, which will allow for embedding current and previous file types as well as external resources will embeded into those zip bundles (images, assets etc). Create a feature which allows managing external assets in the project, the list of assets is stored in "assets.json" in the project (zipped) file. The types of asset that should be supported at this time are images (.png, .jpeg, .gif), .txt (text files), audio (.ogg, .wav), video (.ogv), vector graphics (.svg), scenes (.smil), and create a feature to manage assets in a speciffic .spa project, an asset manager that is presented as a dockable pane default docked to the top right below the swatches.
